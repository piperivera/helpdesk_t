...
function priorityChipClass(p: TicketDetail["priority"]) {
...
export default function TicketDetailPage() {
  ...
  if (slaHrs !== null) {
    if (slaHrs < 0) {
      slaText = `Vencido ${Math.abs(slaHrs)}h`;
      slaChip = "chip-red";
    } else if (slaHrs <= 4) {
      slaText = `${slaHrs}h`;
      slaChip = "chip-amber";
  ...
  const isResolverOrAdmin = session.user.role === "resolver" || session.user.role === "admin";
      ...
      <header className="topbar">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-ink">Ticket {ticket.number}</p>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto space-y-4 animate-fadeIn">
          ...
          <section className="card">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <span className="chip-muted">{ticket.type}</span>
              <span className={statusChipClass(ticket.status)}>{statusLabel(ticket.status)}</span>
              <span className={slaChip}>SLA {slaText}</span>
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 md:grid-cols-[minmax(0,2fr),minmax(0,1.3fr)] gap-4">
            <div className="card space-y-3">
              ...
              <p className="text-sm text-ink/80 whitespace-pre-line">{ticket.description}</p>

              <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-[11px] text-muted mt-2">
                <div>
                  <div className="font-semibold text-ink text-xs mb-0.5">Área</div>
                  <div>{ticket.area}</div>
                </div>
                <div>
                  <div className="font-semibold text-ink text-xs mb-0.5">Creado</div>
                  <div>{formatDate(ticket.createdAt)}</div>
                </div>
                <div>
                  <div className="font-semibold text-ink text-xs mb-0.5">Actualizado</div>
            ...
            <div className="card space-y-3">
              <div>
                ...
                <div className="mt-2 text-xs text-ink/80 space-y-0.5">
                  <div className="font-semibold">{ticket.requester.name || ticket.requester.email}</div>
                  <div>{ticket.requester.email}</div>
                  {ticket.requester.area && <div className="text-muted">Área: {ticket.requester.area}</div>}
                </div>
              </div>
              ...
              <div className="border-t border-gray-100 pt-3 space-y-1 text-xs">
                ...
                {ticket.assignee ? (
                  <div className="text-ink/80 space-y-0.5">
                    ...
                    {ticket.assignee.area && <div className="text-muted">Área: {ticket.assignee.area}</div>}
                  </div>
                ) : (
              ...
              <div className="border-t border-gray-100 pt-3 text-xs space-y-1">
                ...
                {ticket.dueAt && (
                  <div className="text-[11px] text-muted">Vence: {formatDate(ticket.dueAt)}</div>
                )}
              </div>
            </div>
          </section>
            ...
            <section className="card space-y-4">
              <div>
                ...
                <p className="card-subtitle mt-1">
                  Cambia el estado y registra la acción realizada. Los comentarios quedan en el historial.
                </p>
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-3">
                ...
                <div className="flex-1">
                  <label className="block text-xs font-medium text-muted mb-1">Descripción / Comentario</label>
                  <textarea
                    rows={3}
              ...
              <div className="mt-2 border-t border-gray-100 pt-4 space-y-3">
                <div>
                  <h3 className="text-sm font-semibold text-ink">Adjuntar archivos</h3>
                  <p className="card-subtitle mt-1">Adjunta capturas o documentos relacionados con la gestión.</p>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-3 md:items-center">
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => fileInputRef.current?.click()} className="btn-ghost btn-sm">
                      Seleccionar
                    </button>
                    <button
                      type="button"
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="button" onClick={handleUpdate} className="btn-primary btn-sm">
                  Guardar
                </button>
              </div>
            </section>
                ...
                {ticket.attachments.map((att) => (
                  <li key={att.id} className="flex justify-between gap-2">
                    ...
                    <span className="text-[11px] text-muted">
                      {Math.round(att.fileSize / 1024)} KB · {new Date(att.uploadedAt).toLocaleString()}
                    </span>
                  </li>
                ))}
              </ul>
                ...
                {ticket.events.map((ev) => (
                  <li key={ev.id} className="flex gap-2 text-xs">
                    ...
                    <div>
                      ...
                      {ev.description && <div className="text-ink/80">{ev.description}</div>}
                    </div>
                  </li>
                ))}
              </ul>
